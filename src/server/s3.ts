...
export const createS3Client: () => S3Client = (): S3Client =>
...
export const parseObjectKeyFromURI = (uri: string): string => {
  ...
  for (let i = 2; i <= uriAry.length; i++) {
    if (uriAry[i]) Key += uriAry[i];
    if (i === uriAry.length - 1) break;
    Key += '/';
  }
  return Key;
};
...
export const handleS3ObjectPipe = async (
  s3object: S3Object,
  ...
  const objectGetResponse: GetObjectCommandOutput = await createS3Client()
    .send(new GetObjectCommand(s3object))
    .catch((r) => r);

  if (
    objectGetResponse.$metadata.httpStatusCode &&
    objectGetResponse.$metadata.httpStatusCode !== 200 &&
    objectGetResponse.$metadata.httpStatusCode !== 206
  ) {
    notFound(response);
    return;
  }
