...
import {
  ...
  S3PipeServerRequestListenerInterface,
} from './interfaces';
...
const handleHttpOptionsMethod = async (
  response: ServerResponse,
): Promise<void> =>
  await handleHttpResponse(response, async (res) => {
    res.writeHead(200, {
      Allow: 'OPTIONS, GET',
    });
    res.end();
...
const listenerHandler = async (
  ...
  response: ServerResponse,
  customHandler: S3PipeServerCustomHandlers,
): Promise<any> => {
  const { authenticate, resolveObject }: S3PipeServerCustomHandlers =
    customHandler;
...
export class S3PipeServerRequestListener
  implements S3PipeServerRequestListenerInterface
{
  ...
  createHandler(): RequestListener {
    return (request: IncomingMessage, response: ServerResponse) => {
      ...
      listenerHandler(request, response, this.customHandlers || {}).catch(
        (e) => {
          console.log(e);
        },
      );
